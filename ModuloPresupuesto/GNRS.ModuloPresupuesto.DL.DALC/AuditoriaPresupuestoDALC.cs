...
namespace GNRS.ModuloPresupuesto.DL.DALC
{
    public class AuditoriaPresupuestoDALC
    {
        ...
        public AUDITORIA_PRESUPUESTO obtenerAuditoriaProyectadaXTipoAccion_actividad(int id_personalProyectado, string tipoAccion)
        {
            try
            {
                AUDITORIA_PRESUPUESTO obj = new AUDITORIA_PRESUPUESTO();
                var context = new PresupuestoDBEntities();
                obj = (from p in context.AUDITORIA_PRESUPUESTO
                       where p.id_actividad_proyectada == id_personalProyectado && p.tipo_accion == tipoAccion
                       select p).First();
