...
namespace GNRS.ModuloPresupuesto.DL.DALC
{
    public class ActividadRRHHDALC
    {
        ...
        public List<PRESUPUESTO_ACTIVIDAD_PROYECTADA> listarActividadProyectada(PRESUPUESTO_ACTIVIDAD_PROYECTADA objactividad)
        {
            
            try
            {
                List<PRESUPUESTO_ACTIVIDAD_PROYECTADA> lista = new List<PRESUPUESTO_ACTIVIDAD_PROYECTADA>();
                var context = new PresupuestoDBEntities();
                var prueba = (from p in context.PRESUPUESTO_ACTIVIDAD_PROYECTADA                              
                              where (p.mes_inicio == objactividad.mes_inicio || objactividad.mes_inicio == 0) 
                              && (p.mes_fin == objactividad.mes_fin || objactividad.mes_fin == 0) 
                              && (p.anio_inicio == objactividad.anio_inicio || objactividad.anio_inicio == 0)
                              && (p.anio_fin == objactividad.anio_fin || objactividad.anio_fin == 0)
                              
                             // && (p.presupuesto_aprobado == objactividad.presupuesto_aprobado || objactividad.presupuesto_aprobado == "V")
                              select p
 
                              //select new { p.id_actividad_proyectada, p.codigo_actividad_proyectada, a.nombre_actividad, p.monto_actividad, p.tipo_moneda, p.fecha_creacion, p.presupuesto_aprobado, p.mes_inicio, p.anio_inicio, p.mes_fin, p.anio_fin }
        ...
        public List<PRESUPUESTO_ACTIVIDAD_PROYECTADA> listarActividadProyectadaXEstado(PRESUPUESTO_ACTIVIDAD_PROYECTADA objactividad)
        {
            ...
            try
            {
                List<PRESUPUESTO_ACTIVIDAD_PROYECTADA> lista = new List<PRESUPUESTO_ACTIVIDAD_PROYECTADA>();
                var context = new PresupuestoDBEntities();
                lista = (from p in context.PRESUPUESTO_ACTIVIDAD_PROYECTADA
                              where (
                              p.mes_inicio == objactividad.mes_inicio || objactividad.mes_inicio == 0)
                              && (p.mes_fin == objactividad.mes_fin || objactividad.mes_fin == 0)
                              && (p.anio_inicio == objactividad.anio_inicio || objactividad.anio_inicio == 0)
                              && (p.anio_fin == objactividad.anio_fin || objactividad.anio_fin == 0)
                              && (p.presupuesto_aprobado == objactividad.presupuesto_aprobado || objactividad.presupuesto_aprobado == "")
                              && p.id_periodo_presupuesto==objactividad.id_periodo_presupuesto
                              select p                              
                            ).ToList();
        ...
        public Boolean editarEstadoActividad(int id, string nuevoEstado)
        {
            try
            {
                var context = new PresupuestoDBEntities();
                PRESUPUESTO_ACTIVIDAD_PROYECTADA obj = (from a in context.PRESUPUESTO_ACTIVIDAD_PROYECTADA
                               where a.id_actividad_proyectada == id
                               select a).First();


                obj.presupuesto_aprobado = nuevoEstado;
                context.SaveChanges();
                return true;
        ...
        public PRESUPUESTO_ACTIVIDAD_PROYECTADA obtenerActividadProyectXCodigo(int cod)
        {
            try
            {
                var context = new PresupuestoDBEntities();
                PRESUPUESTO_ACTIVIDAD_PROYECTADA actividad = new PRESUPUESTO_ACTIVIDAD_PROYECTADA();

                actividad = (from a in context.PRESUPUESTO_ACTIVIDAD_PROYECTADA
                             where a.id_actividad_proyectada == cod
                             select a).First();
